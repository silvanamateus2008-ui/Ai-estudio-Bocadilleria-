...
const PROMPT_TABS = [
  { id: 'ALL', title: 'Documento Completo (.md)', code: '06_PROMPTS' },
  { id: 'SCR-01', title: 'SCR-01: Autenticación & Onboarding', code: 'PROMPT 1' },
  { id: 'SCR-02', title: 'SCR-02: Dashboard Principal 4 KPIs', code: 'PROMPT 2' },
  { id: 'SCR-03', title: 'SCR-03: Catálogo Tabla y Kanban', code: 'PROMPT 3' },
  { id: 'SCR-04', title: 'SCR-04: Detalle 360 FACT-8942', code: 'PROMPT 4' },
  { id: 'SCR-05', title: 'SCR-05: Modal Wizard POS Pasos', code: 'PROMPT 5' },
  { id: 'SCR-06', title: 'SCR-06: Configuración & Perfil', code: 'PROMPT 6' },
  { id: 'SCR-07', title: 'SCR-07: Consola & Calendario', code: 'PROMPT 7' },
];

export const StitchPromptsModal: React.FC<StitchPromptsModalProps> = ({
  isOpen,
  onClose,
  onNavigateToScreen,
}) => {
  ...
  const getPromptContent = (tab: string): string => {
    switch (tab) {
      case 'SCR-01':
        return `Create an authentic Warm Retro Vintage authentication and onboarding screen for 'Fábrica de Bocadillos La Esperanza S.A.S.' (Vélez, Santander, Colombia) running on Payara Server 7.x JVM. 
...
- Success Toast: Floating notification at bottom-right in olive #283618 with cream text '¡Sesión Autorizada! ✓ Bienvenido Don Carlos Ruiz. Cargando Libro Mayor...'.`;
      ...
      case 'SCR-02':
        return `Design a comprehensive administrative Dashboard for 'Fábrica de Bocadillos La Esperanza' (Vélez, Santander) displaying real-time agroindustrial inventory, production batches, and wholesale orders.
...
- Error Alert: Crimson top banner '#FFDAD6' with warning: 'Advertencia de Comunicación de Lotes: Error al sincronizar stock con el servidor MySQL Vélez-Central (Error 504 Gateway Timeout)' with 'Reintentar Ahora'.`;
      ...
      case 'SCR-03':
        return `Build the Product Catalog and Inventory Management view for 'Fábrica de Bocadillos La Esperanza' with dual Table and Kanban views, batch traceability, and agroindustrial stock tracking.
...
- Error Alert: Crimson banner in #FFDAD6 with title 'Acción Denegada en Lote Activo' and message: '✕ No se pudo eliminar el registro seleccionado. La referencia está asociada a un despacho y factura de venta activa (#FAC-2024-819).'`;
      ...
      case 'SCR-04':
        return `Create the 360 Comprehensive Order Record and Dispatch Dossier for 'FACT-2026-8942' of Fábrica de Bocadillos La Esperanza, featuring invoice breakdowns, compliance gauges, tabs, and real-time shipment timelines.
...
- Error Alert: Collapsible banner in #FFDAD6 with title 'Aviso de Infraestructura Contable' and message: 'El servidor de transacciones Payara 7 Micro-Cluster #4 no responde a la validación remota DIAN' with retry button.`;
      ...
      case 'SCR-05':
        return `Design a multi-step Modal Wizard for wholesale Point-of-Sale (POS) order dispatch creation at 'Fábrica de Bocadillos La Esperanza', featuring live pricing math, stock validation, and HTTP-session persistence.
...
- Error Alert: Warning strip inside modal stating: '⚠️ Stock Insuficiente: Solo quedan 2 cajas disponibles para Caja Regalo Bocadillo Surtido en la bodega de Vélez (Solicitadas: 10)' with one-click 'Ajustar a 2 Cajas' button.`;
      ...
      case 'SCR-06':
        return `Create the System Settings and User Profile Administration screen for 'Fábrica de Bocadillos La Esperanza' with MySQL connection parameters, toggle switches, and Don Carlos Ruiz's master profile.
...
- Error Alert: Top crimson banner '#FFDAD6' with warning: 'Error de Autenticación de Nodo Local: Error al cambiar la contraseña: La contraseña actual no coincide con los registros del nodo Vélez (Payara AuthRealm v7.4)'.`;
      ...
      case 'SCR-07':
        return `Build the Specialized Agroindustrial Logistics Console and Dispatch Calendar for 'Fábrica de Bocadillos La Esperanza' (Vélez, Santander) integrating a monthly broadsheet calendar, real-time Payara 7.x event stream, and JasperReports generation cards.
...
- Error Alert: High-priority crimson banner in #FFDAD6 with title 'Fallo de Comunicación con el Servidor Payara 7.x' and message: 'No se pudo compilar el archivo JasperReports en el nodo remoto core-payara7-velez.prod' with retry button.`;
      ...
      default:
        // Full 06_PROMPTS_GOOGLE_STITCH.md
        return `# 06_PROMPTS_GOOGLE_STITCH.md
...
(Ver las pestañas superiores para inspeccionar los 7 prompts individuales completos con especificaciones de los 4 estados UI obligatorios).`;
    }
  };

  const handleCopy = () => {
    const text = getPromptContent(activeTab);
  ...
  const handleDownload = () => {
    const text = getPromptContent('ALL');
    const blob = new Blob([text], { type: 'text/markdown;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = '06_PROMPTS_GOOGLE_STITCH.md';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#283618]/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-fadeIn">
      <div className="bg-[#fefae0] w-full max-w-5xl rounded border-[3px] border-[#bc6c25] retro-shadow-lg flex flex-col max-h-[90vh] overflow-hidden my-auto">
        {/* Modal Top Bar */}
        <div className="bg-[#283618] text-[#fefae0] p-3 sm:p-4 border-b-2 border-[#dda15e] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-[#bc6c25] rounded text-white retro-shadow-sm">
              <FileText className="w-5 h-5" />
            </div>
            <div>
              <span className="text-[10px] font-mono tracking-wider text-[#dda15e] uppercase font-bold">
                GOOGLE STITCH PROMPT SUITE // ESTILO #10
              </span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 bg-[#dda15e] hover:bg-[#bc6c25] text-[#1d1c0d] hover:text-white px-3 py-1.5 rounded text-xs font-semibold retro-shadow-sm transition-colors"
        ...
        <div className="bg-[#f4f1de] border-b border-[#bc6c25] px-3 pt-2 flex items-center gap-1 overflow-x-auto">
          {PROMPT_TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-sans whitespace-nowrap rounded-t transition-all flex items-center gap-1.5 ${
                  isSelected
                    ? 'bg-[#fefae0] text-[#8f4a00] font-bold border-t-2 border-l-2 border-r-2 border-[#bc6c25] shadow-xs'
                    : 'text-[#544438] hover:text-[#1d1c0d] hover:bg-[#fefae0]/50'
                }`}
              >
                <span className="font-mono text-[10px] text-[#bc6c25] font-semibold">
                  {tab.code}
                </span>
                <span>{tab.title}</span>
              </button>
        ...
        {/* Content Area */}
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 font-mono text-xs bg-[#fefae0] text-[#1d1c0d]">
          <div className="flex items-center justify-between mb-3 bg-[#f4f1de] p-2.5 rounded border border-[#bc6c25]">
            <div className="flex items-center gap-2">
              <span className="font-sans text-xs text-[#544438]">
                Vista previa de texto en bloque listo para copiar a{' '}
                <strong className="text-[#8f4a00]">stitch.withgoogle.com</strong>
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleCopy}
                className="bg-[#bc6c25] hover:bg-[#8f4a00] text-white px-3 py-1 rounded text-xs font-sans font-bold flex items-center gap-1.5 retro-shadow-sm retro-press transition-colors"
              >
                {copied ? (
                  <>
                    <Check className="w-3.5 h-3.5 text-[#d7e9bd]" />
          ...
          <div className="bg-white p-4 rounded border-2 border-[#bc6c25] shadow-inner overflow-x-auto text-[#1d1c0d] leading-relaxed select-text max-h-[50vh]">
        ...
        {/* Modal Footer */}
        <div className="bg-[#f4f1de] p-3 border-t border-[#bc6c25] flex flex-wrap items-center justify-between gap-2 text-xs text-[#544438] font-sans">
          <span>
            Archivo físico generado en raíz: <code className="font-mono text-[#8f4a00]">/06_PROMPTS_GOOGLE_STITCH.md</code>
          </span>
          <button
            onClick={onClose}
            className="px-4 py-1.5 bg-[#283618] hover:bg-[#1b2413] text-[#fefae0] font-bold rounded retro-shadow-sm transition-colors ml-auto"
          >
            Entendido / Cerrar
          </button>
        </div>
